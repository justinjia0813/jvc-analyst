...
| skill | 一句话 | 状态 |
|-------|--------|------|
| `/prescreen` | 给素材，快速过一遍核心问题，输出结构化初筛纪要 | 待建 |
...
**场景**：对某个赛道需要快速建立认知框架，或为可比公司估值提供参照系。
...
**LLM 做什么**：
- 搜索整理公开信息，输出结构化赛道报告
- 覆盖：行业定义与边界 → 价值链拆解 → 主要玩家分层 → 增长驱动力 → 监管与政策 → 核心风险
- 整理可比公司表（公司名、阶段、融资、估值、核心指标、差异化定位）

**输出**：
- 赛道图谱（Markdown）
- 可比公司对比表
- **硬约束**：所有数据点标注来源；找不到来源标 `[未核实]`；不编造市场规模数字
