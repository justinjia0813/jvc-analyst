...
| Trigger Lab | warn | 15 个 deterministic trigger cases | 补模型实际运行的盲测和对抗路由保留集 |
| Output Lab | pass | 15 个输出契约；deal-flow 临时项目库端到端自检；5 个 core 真实路径案例；2 个基线/候选案例；2 个已裁决盲审案例；report 的 13 页文件渲染与视觉检查 |  |
