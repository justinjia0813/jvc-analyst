...
- 工作簿进入 `05-market-sizing.xlsx`；模型假设、公式、来源和敏感项不按级别省略。

## 反合理化约束

- “市场显然很大，不必精确测算” → 市场规模是估值输入；不确定就给区间和缺口，不能用形容词代替。
- “行业惯例可以直接当参数” → 每个参数写数值、单位、时间、地域和来源；无来源不得进入基准情景。
- “两种方法结果接近就算交叉验证” → Top-Down 与 Bottom-Up 使用重叠输入时不独立，必须在正交检查中揭示。
- “模型算出数字，所以数字可靠” → 输出可靠性取决于输入和口径；保留敏感性与证伪条件。
