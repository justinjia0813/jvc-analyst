...
- 纯运营性的独立纪要整理不进入研究分级；纪要一旦进入项目研究链，项目至少按 L1 管理，并读取已有 `spec/CONTEXT.md` 与 `spec/hypotheses.md`。
- 当前项目仍为 L0 时，执行前先说明会新增事实层材料与维护成本，由用户确认是否升级；确认后生成纪要，但不自动创建 `spec/`。
- 原始逐字稿继续留在 `00-source/`，纪要是结构化事实层入口，不替代原始材料。

## 反合理化约束

- “逐字稿太长，摘要即可” → 压缩即立场；保留关键数字、因果链、例子、不确定性与可复核原话。
- “受访者说得很肯定，所以可写成事实” → 仍标 `[创始人自述]`、客户自述或专家观点，除非有独立证据。
- “回答不完整，可以根据上下文补全” → 不补写受访者未表达的内容；把缺口转为后续追问。
- “纪要流畅比保留矛盾更重要” → 回避、前后矛盾和口径变化必须保留在待验证部分。
