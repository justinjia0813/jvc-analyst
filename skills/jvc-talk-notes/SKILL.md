...
- 纯运营性的独立问答纪要整理不进入研究分级；纪要一旦进入项目研究链，项目至少按 L1 管理，并读取已有 `spec/CONTEXT.md` 与 `spec/hypotheses.md`。
- 当前项目仍为 L0 时，执行前先说明会新增事实层材料与维护成本，由用户确认是否升级；确认后生成纪要，但不自动创建规格文件。
- 原始逐字稿继续留在 `00-source/`，问答重排必须能回到原问题和原回答。

## 反合理化约束

- “按主题合并更清晰，可以合并相近问题” → 只有语义相同才合并；保留原问题意图和回答边界。
- “完整回答可以补上合理的上下文” → 不替受访者推导；推断写入用户观察或待验证点。
- “一句话结论比原话更有用” → 重要数字、条件、例子与不确定性不得因压缩丢失。
- “问答形式天然客观” → 重排仍是编辑判断；矛盾、回避和无来源数字必须显式保留。
